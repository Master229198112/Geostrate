...
import { Key, ChevronRight, Presentation, Globe, Crosshair, Moon, Sun, LogIn, LogOut, User as UserIcon } from 'lucide-react';
...
function AppContent() {
  ...
  const [viewState, setViewState] = useState<'input' | 'progress' | 'results'>('input');
  ...
  const handleAnalyze = async () => {
    ...
    if (!guestMobile.trim()) { setError('Mobile number is required.'); return; }
    if (!guestEmail.trim()) { setError('Email is required.'); return; }
    if (!guestCountry.trim()) { setError('Country is required.'); return; }
    if (!apiKey.trim() && !hasStoredKey) { setError('API Key is required.'); return; }
    if (!problem.trim()) { setError('Strategic Target Input is required.'); return; }
    if (problem.length > MAX_CHARS) { setError(`Input too long. Maximum ${MAX_CHARS} characters.`); return; }
    ...
    try {
      ...
      try {
        ...
        throw new Error(response.ok ? 'Invalid response from server.' : `Server error (${response.status}): ${text.substring(0, 200)}`);
      }
  ...
  if (currentPath === '/admin') {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans">
        <Suspense fallback={<div className="flex h-screen items-center justify-center text-slate-500">Loading Admin...</div>}>
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-200 font-sans selection:bg-blue-500/20">
      ...
      <header className="border-b border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md sticky top-0 z-10 shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-lg font-semibold text-slate-800 dark:text-slate-100 tracking-tight uppercase">Geostrate</h1>
          </div>
          <div className="flex items-center gap-2 sm:gap-4">
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors" title="Toggle dark mode">
            ...
            {viewState === 'results' && hasStoredKey && (
              <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 rounded-sm px-3 py-1.5">
                ...
                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Key Active</span>
              </div>
            )}
            {/* User Auth Section */}
            {user ? (
              <div className="flex items-center gap-2">
                <div className="hidden sm:flex items-center gap-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-sm px-3 py-1.5 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors" onClick={() => setShowProfile(true)}>
                  <UserIcon className="w-4 h-4 text-blue-500" />
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-200 max-w-[80px] truncate">{user.name}</span>
                  {user.plan ? (
                    <span className="text-[9px] font-bold bg-blue-600 text-white px-1.5 py-0.5 rounded-sm uppercase tracking-widest">{user.plan}</span>
                  ) : (
                    <span className="text-[9px] font-bold bg-slate-300 dark:bg-slate-600 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded-sm uppercase tracking-widest">Free</span>
                  )}
                </div>
                <button onClick={logout} className="p-2 rounded-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors" title="Logout">
                  <LogOut className="w-4 h-4 text-slate-400 hover:text-rose-500" />
                </button>
              </div>
            ) : (
              <button onClick={() => setShowAuth(true)} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-sm text-xs font-bold uppercase tracking-widest transition-colors">
                <LogIn className="w-3.5 h-3.5" /> Login
              </button>
            )}
            <img src="/logo.png" alt="Woxsen University" className="h-10 object-contain hidden sm:block" />
          </div>
        </div>
      </header>
        ...
        <main className="flex-grow flex items-center justify-center px-4 py-12">
          <div className="w-full max-w-lg">
            ...
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm p-6 shadow-sm space-y-4">
              {/* Name */}
              <div>
                <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Name *</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Mobile No. *</label>
                  <input
                    type="tel"
                    className="w-full bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-sm px-3 py-2.5 text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Email ID *</label>
                  <input
                    type="email"
                    className="w-full bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-sm px-3 py-2.5 text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Country *</label>
                  <input
                    type="text"
                    className="w-full bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-sm px-3 py-2.5 text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                    placeholder="India"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">
                    Gemini API Key * {hasStoredKey && <span className="text-emerald-500 ml-1">(stored)</span>}
                  </label>
                  <input
                    type="password"
                    className="w-full bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-sm px-3 py-2.5 text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">
                  <Crosshair className="w-3 h-3 inline mr-1" />Strategic Target Input *
                </label>
                <div className="relative">
                  ...
                  />
                  <span className={`absolute bottom-2 right-2 text-[10px] font-mono ${problem.length > MAX_CHARS * 0.9 ? 'text-rose-500' : 'text-slate-400'}`}>
              ...
              {/* Error */}
              {error && <p className="text-rose-500 text-xs font-mono">{error}</p>}
              ...
              >
                Generate Intelligence Brief <ChevronRight className="w-4 h-4" />
              </button>
      ...
      {viewState === 'progress' && (
        <main className="flex-grow flex items-center justify-center px-4 py-12">
          <div className="w-full max-w-md text-center">
            ...
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Generating Intelligence Brief</h2>
            <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-8">Please wait while we analyze your input</p>
              ...
              <div className="space-y-4">
                {PROGRESS_STEPS.map((step, i) => (
                  <div key={i} className={`flex items-center gap-3 transition-all duration-500 ${
                    i < progressStep ? 'text-emerald-600 dark:text-emerald-400' : i === progressStep ? 'text-blue-600 dark:text-blue-400' : 'text-slate-300 dark:text-slate-600'
                  }`}>
                    {i < progressStep ? (
                    ...
                    <span className="text-xs font-mono font-semibold">{step}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {error && (
              <div className="mt-4 p-3 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-700 rounded-sm">
                <p className="text-rose-600 dark:text-rose-400 text-xs font-mono">{error}</p>
                <button onClick={() => setViewState('input')} className="mt-2 text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest">← Back to Input</button>
              </div>
            )}
          </div>
        </main>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              {pd?.executiveSummary?.escalationTimeline && (
                <ErrorBoundary fallbackTitle="Escalation Timeline Error">
                  <EscalationTimeline events={pd.executiveSummary.escalationTimeline} />
                </ErrorBoundary>
              )}
            </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              {results && simResults && simInputs ? (
                <>
                  ...
                  <div className="flex items-center justify-between bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-sm shadow-sm">
                    <div className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-widest font-mono">
                      System Status: <span className="text-emerald-600 dark:text-emerald-400">Active Monitoring</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setShowPresentation(true)} className="text-xs flex items-center gap-1.5 text-blue-600 dark:text-blue-400 hover:text-blue-500 transition-colors bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 px-3 py-1.5 rounded-sm uppercase tracking-wider font-bold">
                        <Presentation className="w-3.5 h-3.5" /> Executive Briefing
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {pd?.part8_StrategicQA && pd.part8_StrategicQA.length > 0 && (
                    ...
                    {pd?.part9_DecisionArchitecture && (
                      <ErrorBoundary fallbackTitle="Decision Architecture Error">
                        <DecisionArchitectureCard data={pd.part9_DecisionArchitecture} />
                      </ErrorBoundary>
                    )}
                ...
                <div className="h-full min-h-[600px] border border-slate-200 dark:border-slate-700 border-dashed rounded-sm flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-800/50">
                  ...
                  <p className="text-xs uppercase tracking-widest font-mono">Processing Strategic Data</p>
                </div>
              )}
            </div>
          </div>
      ...
      <footer className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-8 mt-auto">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-widest">
            Strategic Intelligence Platform <span className="text-slate-600 dark:text-slate-300 font-bold ml-2">v4.0</span>
          </div>
          <div className="flex items-center gap-6 text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-bold">
            ...
            <button onClick={() => setActiveModal('terms')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Terms of Service</button>
            <button onClick={() => setActiveModal('ethics')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Research Ethics</button>
          </div>
        </div>
      </footer>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setActiveModal(null)}>
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-xs text-slate-600 space-y-4 leading-relaxed font-mono">
              {activeModal === 'privacy' && (
                <>
                  <p><strong>1. Data Collection:</strong> Geostrate collects coordination problem descriptions and Gemini API keys strictly for the purpose of computing the CIPF v4.0 framework. API keys are not stored persistently and are only used during the active session.</p>
                  <p><strong>2. Data Usage:</strong> The data provided is processed using Google's Gemini API to extract entities and map variables. We do not use your data to train our own models.</p>
                  <p><strong>3. Third-Party Services:</strong> This application relies on the Google Gemini API. By using this service, you also agree to Google's data processing terms.</p>
                  <p><strong>4. Security:</strong> We implement reasonable security measures to protect your inputs during transit. However, no internet-based service is 100% secure.</p>
                </>
              )}
              {activeModal === 'terms' && (
                <>
                  <p><strong>1. Acceptance of Terms:</strong> By accessing and using Geostrate, you accept and agree to be bound by the terms and provision of this agreement.</p>
                  <p><strong>2. Use License:</strong> Permission is granted to temporarily use this application for personal, non-commercial, or academic research purposes.</p>
                  <p><strong>3. Disclaimer:</strong> The materials on Geostrate are provided on an 'as is' basis. AI Research Centre Woxsen University makes no warranties, expressed or implied, and hereby disclaims and negates all other warranties including, without limitation, implied warranties or conditions of merchantability, fitness for a particular purpose, or non-infringement of intellectual property or other violation of rights.</p>
                  <p><strong>4. Limitations:</strong> In no event shall AI Research Centre Woxsen University or its suppliers be liable for any damages arising out of the use or inability to use the materials on Geostrate.</p>
                </>
              )}
              {activeModal === 'ethics' && (
                <>
                  <p><strong>1. Purpose:</strong> Geostrate is developed as a research tool to explore Evidence-Based Coordination Intelligence. Its primary goal is to advance academic understanding of complex coordination problems.</p>
                  <p><strong>2. Bias and Fairness:</strong> The CIPF v4.0 engine and the underlying LLM (Gemini) may exhibit biases present in their training data. Users should interpret the results as analytical aids, not absolute truths.</p>
                  <p><strong>3. Transparency:</strong> We strive to make the CIPF mathematical framework deterministic and transparent. The variable mapping process, however, relies on probabilistic LLM outputs.</p>
                  <p><strong>4. Responsible Use:</strong> Users are expected to use this tool responsibly. It should not be used to make critical decisions without human oversight and domain expertise.</p>
                </>
              )}
            </div>
            <div className="mt-6 flex justify-end">
              <button onClick={() => setActiveModal(null)} className="bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 px-4 py-2 rounded-sm text-xs font-bold uppercase tracking-widest transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAuth && (
        <Suspense fallback={<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40"><div className="text-white text-xs uppercase tracking-widest">Loading...</div></div>}>
      ...
      {showPresentation && results && (
        <Suspense fallback={<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40"><div className="text-white text-xs uppercase tracking-widest">Loading...</div></div>}>
          <PresentationDeck results={results} onClose={() => setShowPresentation(false)} />
        </Suspense>
      )}
    </div>
  );
